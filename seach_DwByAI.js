...
const CONFIG = {
  ...
  SHEET_SOURCE: "1DRteBSFT1cj4R_OUPMoDxeLMzAIJexWF3HPT-rpMOoM",
...
/**
 * CHUYỂN ĐỔI URL/ID/MÃ BẢN VẼ SANG BASE64 DỮ LIỆU SẠCH (3-LAYER FALLBACK)
 * @param {string} urlOrId
 * @returns {string} Base64 Data URL
 */
function getBase64FromUrlOrId(urlOrId) {
  try {
    if (!urlOrId || String(urlOrId).indexOf("icons8") !== -1) return "";
    const str = String(urlOrId).trim();
    const token = ScriptApp.getOAuthToken();

    // Layer 1: Nếu là URL trực tiếp
    if (str.startsWith("http")) {
      try {
        const res = UrlFetchApp.fetch(str, {
          headers: { Authorization: "Bearer " + token },
          muteHttpExceptions: true
        });
    ...
    const fileIdMatch = str.match(/[-\w]{25,}/);
    if (fileIdMatch) {
      const fileId = fileIdMatch[0];
      try {
        const driveRes = UrlFetchApp.fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`, {
          headers: { Authorization: `Bearer ${token}` },
          muteHttpExceptions: true
        });
        if (driveRes.getResponseCode() === 200) {
          const blob = driveRes.getBlob();
          if (blob.getBytes().length > 100) {
            return "data:" + blob.getContentType() + ";base64," + Utilities.base64Encode(blob.getBytes());
          }
        }
    ...
    const cleanCode = str.replace(/\.[^/.]+$/, "").toLowerCase();
    const driveMap = fetchImagesFromDrive([cleanCode]);
    if (driveMap && driveMap[cleanCode]) {
      return driveMap[cleanCode];
    }
